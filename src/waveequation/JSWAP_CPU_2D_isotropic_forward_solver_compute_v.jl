...
"
@parallel function JSWAP_CPU_2D_isotropic_forward_solver_compute_v(dt,dx,dy,rho,beta,
    v1_iph_j,v2_i_jph,
    sigmas11_ip1_j_1,
    sigmas22_i_jp1_2,
    ...
    @all(v1_iph_j)=dt./@all(rho) .*((@all(sigmas11_ip1_j_1)-@all(p_ip1_j_1))/dx+
    @all(sigmas12_iph_jph_2)/dy)+
    @all(v1_iph_j)-
    dt*@all(beta) .*@all(v1_iph_j);

    @all(v2_i_jph)=dt./@all(rho) .*(@all(sigmas12_iph_jph_1)/dx+
