...
M22[:]=1*(rickerWave(freq,dt,nt,2))#+rickerWave(freq*2,dt,nt,2)+rickerWave(freq*3,dt,nt,2)+rickerWave(freq*4,dt,nt,2))./4;
...
lambda[:,air] .=20*10^10;#340^2;
mu[:,air] .= 8.1*10^10;#0
rho[:,air] .=7850;#1;

lambda[:,(ny+1).-(air)] .=20*10^10;
mu[:,(ny+1).-(air)]  .=8.1*10^10;#0
rho[:,(ny+1).-(air)]  .=7850;
